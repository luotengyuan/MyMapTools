...
namespace GMapProvidersExt.AMap
{
    public class AMapProvider : AMapProviderBase, RoutingProvider,GeocodingProvider
    {
            ...
            int pageIndex, QueryProgressDelegate queryProgressEvent, ref int totalCount)
        {
            ...
            {
                format += string.Format("&city={0}", HttpUtility.UrlEncode(region));
            }
            if (!string.IsNullOrWhiteSpace(KEY))
            {
                format += string.Format("&key={0}", KEY);
            }
        ...
        public List<Placemark> GetPlacemarksByLocation(PointLatLng location)
        {
            ...
            try
            {
                string reqUrl = string.Format("http://restapi.amap.com/v3/geocode/regeo?output=json&location={0}", location.Lng + "," + location.Lat);
                if (!string.IsNullOrWhiteSpace(KEY))
                {
                    reqUrl += string.Format("&key={0}", KEY);
                }
                ...
                if (jsonObj != null && jsonObj["info"].ToString() == "OK")
                {
                    ...
                    if (regeocode != null)
                    {
                        ...
                        if (addressComponent != null)
                        {
                            ...
                            place.CityCode = addressComponent["citycode"].ToString();
                            place.AdCode = addressComponent["adcode"].ToString();
        ...
        private List<PointLatLng> GetPointsByPlacemark(Placemark placemark)
        {
            ...
            try
            {
                string reqUrl = string.Format("http://restapi.amap.com/v3/geocode/geo?address={0}&output=json", placemark.Address);
                if (!string.IsNullOrWhiteSpace(KEY))
                {
                    reqUrl += string.Format("&key={0}", KEY);
                }
                ...
                {
                    reqUrl += string.Format("&city={0}", placemark.CityName);
                }
                string content = MapRequestTools.GetData(reqUrl);
                JObject jsonObj = JObject.Parse(content);
                if (jsonObj != null && jsonObj["info"].ToString() == "OK")
        ...
        public GMapRoute GetTransitRoute(PointLatLng start, PointLatLng end,string cityName)
        {
            ...
            string url = string.Format("http://restapi.amap.com/v3/direction/transit/integrated?origin={0}&destination={1}&city={2}&output=json", origin, destination, city);
            if (!string.IsNullOrWhiteSpace(KEY))
            {
                    ...
                    if (walk != null)
                    {
                        ...
                        foreach (JObject step in steps)
                        {
                            ...
                            foreach (string pStr in pointsString)
                            {
                                ...
                                PointLatLng p = new PointLatLng(double.Parse(pointStr[1]), double.Parse(pointStr[0]), CoordType.GCJ02);
                                points.Add(p);
                            }
                        }
                    }
        ...
        public GMapRoute GetDrivingRoute(PointLatLng start, PointLatLng end, List<PointLatLng> wayList)
        {
            ...
            if (!string.IsNullOrEmpty(result))
            {
                ...
                if (isOk == "OK")
                {
                    ...
                    string ori = route["origin"].ToString();
                    string des = route["destination"].ToString();
                    JArray paths = (JArray)route["paths"];
                    if (paths == null || paths.Count <= 0)
                    {
                        return null;
                    }
                    JObject path = (JObject)paths[0];
                    if (path != null)
                    {
                        JArray steps = (JArray)path["steps"];
                        foreach (JObject step in steps)
                        {
                            string polyline = step["polyline"].ToString();
                            string[] pointsString = polyline.Split(';');
